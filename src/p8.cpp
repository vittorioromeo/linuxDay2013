...
template<class T1, class T2> bool intersezione(T1& mForma1, T2& mForma2)
{
	return mForma1.dx() >= mForma2.sx() && mForma1.sx() <= mForma2.dx() &&
			mForma1.sotto() >= mForma2.sopra() && mForma1.sopra() <= mForma2.sotto();
}
...
void collisioneTra(Blocco& mBlocco, Pallina& mPallina)
{
	...
	float minIntersezioneY{pallinaDaSopra ? intersezioneDaSopra : 
							intersezioneDaSotto};
	...
	// orizzontalmente; altrimenti, la pallina ha colpito il blocco verticalmente
